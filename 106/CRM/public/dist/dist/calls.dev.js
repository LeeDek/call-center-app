...
(function (Departments) {})(Departments || (Departments = {}));

var sortStatusButton = document.getElementById("sortStatus");
sortStatusButton.addEventListener("click", function () {
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        ...
        case 1:
          ...
          return [4
          /*yield*/
          , fetch("/api/calls/getCallsByStatus", {
            method: "POST",
            headers: {
        ...
        case 3:
          data = _a.sent();
          callsDB = data.callsDB; // Update the table with the sorted data
...
function renderCalls(callsData) {
  var table = document.querySelector(".requests table"); // Clear the existing table rows

  table.innerHTML = ""; // Loop through the calls data and create table rows

  callsData.forEach(function (callsData) {
    var row = document.createElement("tr"); // Create table cells for each column
    ...
    statusCell.classList.add("status", "clickable"); // Append the cells to the row

    row.appendChild(dateCell);
    row.appendChild(deptCell);
    row.appendChild(statusCell); // Append the row to the table body
